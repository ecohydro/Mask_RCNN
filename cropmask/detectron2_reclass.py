...
def tif_dataset_mapper(dataset_dict):
    ...
    image = utils.read_image(dataset_dict["file_name"]) # relies on my own edit that updates the func to be able to read tif imagery with imageio
...
def build_detection_validation_loader(cfg, mapper=None):
    ...
    dataset_dicts = get_detection_dataset_dicts(
        cfg.DATASETS.VALIDATION, # this is the only difference between the valdiation loader and train loader.
        filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
        min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
        if cfg.MODEL.KEYPOINT_ON
        else 0,
        proposal_files=cfg.DATASETS.PROPOSAL_FILES_TRAIN if cfg.MODEL.LOAD_PROPOSALS else None, # won't use this I think but if I do, needs to be changed for valdiation set.
    )
    ...
    sampler_name = cfg.DATALOADER.SAMPLER_TRAIN #valdiation should use same batching and sampling scheme as training (rather than batch of size 1)
...
class Trainer(DefaultTrainer):
    ...
    def _write_validation_metrics(self, metrics_dict: dict):
        ...
        if comm.is_main_process():
            ...
            metrics_dict = {
                "va_l"+k: np.mean([x[k] for x in all_metrics_dict]) for k in all_metrics_dict[0].keys()
            }
    ...
    def run_step(step):
        ...
        self._detect_anomaly(val_losses, val_loss_dict)

        val_metrics_dict = val_loss_dict
