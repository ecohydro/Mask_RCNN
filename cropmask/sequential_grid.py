...
def get_tiles_for_threaded_map(ds, label_boundary_gdf, width, height):
    """
    ...
    chip_list = []
    def get_win(ds, label_boundary_gdf, col_off, row_off, width, height, big_window):
        ...
        if label_boundary_gdf.contains(GeoSeries(window_poly)).values[0]:
            # can remove all tuple constructs later once I confirm this is right instead of custom transform per window
            # custom transforms skew the windows, see question on https://rasterio.groups.io/
