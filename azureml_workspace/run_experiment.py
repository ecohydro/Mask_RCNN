...
custom_env = Environment(name="custom_env")
custom_env.docker.enabled = True
custom_env.docker.base_image = configs['docker_base_image']
custom_env.docker.base_image_registry.address = "cropmaskcontainers.azurecr.io"
custom_env.docker.base_image_registry.username = configs['base_image_registry_username']
custom_env.docker.base_image_registry.password = configs['base_image_registry_password']
exec_environment = Estimator(source_directory=os.getcwd(),
                             entry_script='cropmask/run_cropmask_simple.py',
...
#%%
from azureml.core import Experiment # 1 experiment class per training script
experiment_name = 'test_experiment_runs'

exp = Experiment(workspace=ws, name=experiment_name)
run = exp.submit(config=exec_environment) # 1 run per configuration
